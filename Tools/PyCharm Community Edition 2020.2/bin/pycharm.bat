...
SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\JetBrains\PyCharmCE2020.2\pycharm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\JetBrains\PyCharmCE2020.2\pycharm%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%APPDATA%\JetBrains\PyCharmCE2020.2\pycharm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_PYCHARM_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_PYCHARM.hprof" -Didea.vendor.name=JetBrains -Didea.paths.selector=PyCharmCE2020.2 %IDE_PROPERTIES_PROPERTY%
